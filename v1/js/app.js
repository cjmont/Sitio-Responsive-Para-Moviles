...
try {  
    ...
    btnRegistro.addEventListener("click", function(){cargarRegistro(localStorage.getItem("boolBr"))}, false);
} catch (err){
    console.log(err)
}
...
function cargarEvento(variable, variable2){
    console.log("")
    localStorage.setItem("curso", variable);
    //localStorage.setItem("evento", variable2);
            ...
            console.log("ENTROOOO")
                ...
                if (objeto.tipo === "Curso"){
                    ...
                    try {
                        ...
                        document.getElementById('nombreCurso' + i).innerHTML = objeto.nombre.slice(0,60);
...
function cargarRegistro (boolBr) {
    ...
    if (boolBr == "true"){
        ...
        var prospecto_id = null;
        var registro_participante = true;
        var evento = parseInt(localStorage.getItem("evento"));
    }
    ...
    fetch('http://www.cec.espol.edu.ec/api/registropost', {
      method: 'post',
      headers: {
